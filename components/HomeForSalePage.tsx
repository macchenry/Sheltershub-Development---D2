...
interface HomeForSalePageProps {
  onNavigate: (page: string) => void;
}

// Reusing the Sale Card design
...
const HomeForSalePage: React.FC<HomeForSalePageProps> = ({ onNavigate }) => {
  const [filteredProperties, setFilteredProperties] = useState<Property[]>([]);

  useEffect(() => {
    window.scrollTo(0, 0);
    // Filter specifically for Houses (Homes) that are For Sale (or Sold for demo purposes)
    const homesForSale = allPropertiesList.filter(
        p => (p.status === PropertyStatus.ForSale || p.status === PropertyStatus.Sold) && p.propertyType === 'House'
    );
    
    // Fallback if data generation doesn't produce enough Houses
    let displayList = homesForSale;
    if (displayList.length === 0) {
        displayList = allPropertiesList.slice(0, 4).map(p => ({...p, propertyType: 'House', status: PropertyStatus.ForSale}));
    }
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans">
      <Header onNavigate={onNavigate} activePage="home-for-sale" />

      {/* Page Title Section */}
      <div className="bg-[#0A2B4C] text-white py-16">
        <div className="container mx-auto px-4 text-center">
            <h1 className="text-3xl md:text-4xl font-bold mb-3">Homes for Sale</h1>
            <p className="text-blue-100 text-lg font-light max-w-2xl mx-auto">Discover beautiful houses and homes available for purchase.</p>
        </div>
      </div>
      ...
      <main className="container mx-auto px-4 py-8 -mt-8 relative z-10">
        ...
        {filteredProperties.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white rounded-xl border border-gray-200 shadow-sm mt-8">
                ...
                <h3 className="text-xl font-bold text-[#0A2B4C] mb-2">No homes found</h3>
                <p className="text-gray-500 mb-8 max-w-md mx-auto">We couldn't find any houses for sale at the moment.</p>
                <button 
