...
interface ApartmentForSalePageProps {
...
const ApartmentForSalePage: React.FC<ApartmentForSalePageProps> = ({ onNavigate }) => {
  const [filteredProperties, setFilteredProperties] = useState<Property[]>([]);

  useEffect(() => {
    window.scrollTo(0, 0);
    // Filter specifically for Apartments that are For Sale
    const apartmentsForSale = allPropertiesList.filter(
        p => (p.status === PropertyStatus.ForSale || p.status === PropertyStatus.Sold) && p.propertyType === 'Apartment'
    );
    
    // Fallback if data generation doesn't produce enough Apartments
    let displayList = apartmentsForSale;
    if (displayList.length === 0) {
        displayList = allPropertiesList.slice(0, 4).map(p => ({
            ...p, 
            propertyType: 'Apartment', 
            status: PropertyStatus.ForSale,
            name: p.name.replace('Home', 'Apartment').replace('Villa', 'Penthouse')
        }));
    }
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans">
      <Header onNavigate={onNavigate} activePage="apartment-for-sale" />

      {/* Page Title Section */}
      <div className="bg-[#0A2B4C] text-white py-16">
        <div className="container mx-auto px-4 text-center">
            <h1 className="text-3xl md:text-4xl font-bold mb-3">Apartments for Sale</h1>
            <p className="text-blue-100 text-lg font-light max-w-2xl mx-auto">Find and purchase luxury apartments and condos in prime locations.</p>
        </div>
      </div>
      ...
      <main className="container mx-auto px-4 py-8 -mt-8 relative z-10">
        ...
        {filteredProperties.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white rounded-xl border border-gray-200 shadow-sm mt-8">
                ...
                <p className="text-gray-500 mb-8 max-w-md mx-auto">We couldn't find any apartments for sale at the moment.</p>
                <button 
