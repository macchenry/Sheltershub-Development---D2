...
import { BedIcon, BathIcon, GarageIcon, HeartIcon, LocationIcon, AreaIcon } from './icons/PropertyIcons';
import { formatCurrency } from '../utils/currency';

interface TownhouseForSalePageProps {
  onNavigate: (page: string) => void;
}

// Sale Card for Townhouses
const SalePropertyCard: React.FC<{ property: Property; onClick: () => void }> = ({ property, onClick }) => {
  ...
  const mockLocations = ["Cantonments, Accra", "East Legon Hills", "Airport Residential", "Spintex Road", "Tema Community 25", "Sakumono"];
...
const TownhouseForSalePage: React.FC<TownhouseForSalePageProps> = ({ onNavigate }) => {
  const [filteredProperties, setFilteredProperties] = useState<Property[]>([]);

  useEffect(() => {
    window.scrollTo(0, 0);
    // Filter specifically for Townhouses that are For Sale
    const townhousesForSale = allPropertiesList.filter(
        p => (p.status === PropertyStatus.ForSale || p.status === PropertyStatus.Sold) && p.propertyType === 'Townhouse'
    );
    
    // Fallback if data generation doesn't produce enough
    let displayList = townhousesForSale;
    if (displayList.length === 0) {
        displayList = allPropertiesList.slice(0, 4).map(p => ({
            ...p, 
            propertyType: 'Townhouse', 
            status: PropertyStatus.ForSale,
            name: p.name.replace('Home', 'Townhouse').replace('Villa', 'Terrace')
        }));
    }
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans">
      <Header onNavigate={onNavigate} activePage="townhouse-for-sale" />

      {/* Page Title Section */}
      <div className="bg-[#0A2B4C] text-white py-16">
        <div className="container mx-auto px-4 text-center">
            <h1 className="text-3xl md:text-4xl font-bold mb-3">Townhouses for Sale</h1>
            <p className="text-blue-100 text-lg font-light max-w-2xl mx-auto">Discover premium townhouses for sale in established neighborhoods.</p>
        </div>
      </div>
      ...
      <main className="container mx-auto px-4 py-8 -mt-8 relative z-10">
        ...
        {filteredProperties.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white rounded-xl border border-gray-200 shadow-sm mt-8">
                ...
                <h3 className="text-xl font-bold text-[#0A2B4C] mb-2">No townhouses found</h3>
                <p className="text-gray-500 mb-8 max-w-md mx-auto">We couldn't find any townhouses for sale at the moment.</p>
                <button 
...
export default TownhouseForSalePage;
