...
export const BedIcon: React.FC<{ className?: string }> = ({ className = "w-5 h-5" }) => (
    <img src="https://i.ibb.co/S4DZwNyz/bed.png" alt="Bed icon" className={className} />
);

export const BathIcon: React.FC<{ className?: string }> = ({ className = "w-5 h-5" }) => (
    <img src="https://i.ibb.co/60BSG1pR/bathroom.png" alt="Bathroom icon" className={className} />
);

export const GarageIcon: React.FC<{ className?: string }> = ({ className = "w-5 h-5" }) => (
    <img src="https://i.ibb.co/5hg25082/garage.png" alt="Garage icon" className={className} />
);

export const AreaIcon: React.FC<{ className?: string }> = ({ className = iconClass }) => (
     <svg className={className} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 3.75v4.5m0-4.5h4.5m-4.5 0L9 9M3.75 20.25v-4.5m0 4.5h4.5m-4.5 0L9 15M20.25 3.75h-4.5m4.5 0v4.5m0-4.5L15 9m5.25 11.25h-4.5m4.5 0v-4.5m0 4.5L15 15" />
    </svg>
);

export const HeartIcon: React.FC<{ isFilled?: boolean; className?: string }> = ({ isFilled = false, className = "h-5 w-5" }) => (
    <svg className={className} fill={isFilled ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
