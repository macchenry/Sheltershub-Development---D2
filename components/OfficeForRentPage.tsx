...
import { AreaIcon, GarageIcon, HeartIcon } from './icons/PropertyIcons';
import { formatCurrency } from '../utils/currency';

interface OfficeForRentPageProps {
  onNavigate: (page: string) => void;
}

// Rental Card for Office
...
const OfficeForRentPage: React.FC<OfficeForRentPageProps> = ({ onNavigate }) => {
  const [filteredProperties, setFilteredProperties] = useState<Property[]>([]);

  useEffect(() => {
    window.scrollTo(0, 0);
    // Filter specifically for Commercial properties marked For Rent
    const officesForRent = allPropertiesList.filter(
        p => (p.status === PropertyStatus.ForRent || p.status === PropertyStatus.Sold) && p.propertyType === 'Commercial'
    );

    // Fallback if data generation doesn't produce enough
    let displayList = officesForRent;
    if (displayList.length === 0) {
        // Fallback: convert some properties to 'Commercial' 'For Rent' for demonstration
        displayList = allPropertiesList.slice(0, 4).map(p => ({
            ...p,
            propertyType: 'Commercial',
            status: PropertyStatus.ForRent,
            name: p.name.replace('Home', 'Office Suite').replace('Apartment', 'Workspace')
        }));
    }
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans">
      <Header onNavigate={onNavigate} activePage="office-for-rent" />

      {/* Page Title Section */}
      <div className="bg-[#0A2B4C] text-white py-16">
        <div className="container mx-auto px-4 text-center">
            <h1 className="text-3xl md:text-4xl font-bold mb-3">Offices for Rent</h1>
            <p className="text-blue-100 text-lg font-light max-w-2xl mx-auto">Premium office spaces available for rent in prime business districts.</p>
        </div>
      </div>
      ...
      <main className="container mx-auto px-4 py-8 -mt-8 relative z-10">
        ...
        {filteredProperties.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white rounded-xl border border-gray-200 shadow-sm mt-8">
                ...
                <p className="text-gray-500 mb-8 max-w-md mx-auto">We couldn't find any office spaces for rent at the moment.</p>
                <button
...
export default OfficeForRentPage;
