...
interface HouseForRentPageProps {
...
const HouseForRentPage: React.FC<HouseForRentPageProps> = ({ onNavigate }) => {
  const [filteredProperties, setFilteredProperties] = useState<Property[]>([]);

  useEffect(() => {
    window.scrollTo(0, 0);
    // Filter for Houses that are For Rent
    const housesForRent = allPropertiesList.filter(
        p => (p.status === PropertyStatus.ForRent || p.status === PropertyStatus.Sold) && p.propertyType === 'House'
    );
    // Fallback if data generation doesn't produce enough
    setFilteredProperties(housesForRent.length > 0 ? housesForRent : allPropertiesList.slice(0, 4));
  }, []);

  return (
    <div className="bg-gray-50 min-h-screen font-sans">
      <Header onNavigate={onNavigate} activePage="house-for-rent" />

      {/* Page Title Section */}
      <div className="bg-[#0A2B4C] text-white py-16">
        <div className="container mx-auto px-4 text-center">
            <h1 className="text-3xl md:text-4xl font-bold mb-3">Houses for Rent</h1>
            <p className="text-blue-100 text-lg font-light max-w-2xl mx-auto">Find your ideal home from our selection of rental houses.</p>
        </div>
      </div>
      ...
      <main className="container mx-auto px-4 py-8 -mt-8 relative z-10">
        ...
        {filteredProperties.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white rounded-xl border border-gray-200 shadow-sm mt-8">
                ...
                <h3 className="text-xl font-bold text-[#0A2B4C] mb-2">No houses found</h3>
                <p className="text-gray-500 mb-8 max-w-md mx-auto">We couldn't find any houses for rent at the moment.</p>
                <button 
