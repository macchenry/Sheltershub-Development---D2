...
interface ApartmentForRentPageProps {
...
const ApartmentForRentPage: React.FC<ApartmentForRentPageProps> = ({ onNavigate }) => {
  const [filteredProperties, setFilteredProperties] = useState<Property[]>([]);

  useEffect(() => {
    window.scrollTo(0, 0);
    // Filter specifically for Apartments that are For Rent
    ...
    // Fallback if data generation doesn't produce enough Apartments
    let displayList = apartmentsForRent;
    if (displayList.length === 0) {
        // Fallback: convert some properties to 'Apartment' 'For Rent' for demonstration
        displayList = allPropertiesList.slice(0, 4).map(p => ({
            ...p, 
            propertyType: 'Apartment', 
            status: PropertyStatus.ForRent,
            name: p.name.replace('Home', 'Apartment').replace('Villa', 'Flat')
        }));
    }
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans">
      <Header onNavigate={onNavigate} activePage="apartment-for-rent" />
